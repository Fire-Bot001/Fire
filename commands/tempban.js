...
module.exports = {
    run: async (message, args) => {
        ...
        if (!member) return message.channel.send('Tu dois mentionner le membre à bannir.')
        if (member.id === message.guild.ownerID) return message.channel.send('Tu ne peux pas bannir le propriétaire du serveur.')
        ...
        if (!duration) return message.channel.send('Tu dois indiquer une durée valide.')
